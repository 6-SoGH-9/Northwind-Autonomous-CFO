...
**`effective_date` definition (authoritative, binding on all versions, not just v1.0):** `effective_date` is the canonical Git-history date on which the underlying analytical definition first became part of canonical `main` at the value that version records — for `v1.0`, this is `2026-08-05`, the commit date of `9cfe4ca1a5fe41747110832d9445328c69563f78`. It is **not** the date the registry implementation itself is written, reviewed, or authorized, and it is not the date a Brief authorizing a change is approved. For any future version (`v1.1`, `v2.0`, etc.), `effective_date` must likewise be the date that version's value actually took effect in canonical product behavior (i.e. the commit date of the canonical change that changed the constant), established the same way — by direct git-history inspection at the time that version is registered, never asserted from memory or convenience. This distinction matters for audit integrity: the registry exists to answer "which definition governed this close," and that answer is only correct if `effective_date` tracks when the rule was actually live, not when someone got around to documenting it.
...
1. All three definitions are registered with complete metadata; each has a stable `version` identifier and a git-history-derived `effective_date`, per the authoritative definition in 3.2 (date the value went live in canonical `main`, never the registry's own authoring/implementation date).
